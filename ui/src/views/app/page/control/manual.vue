            ...
            <b-row>
              <b-col md="4">
                <div v-if="controlData[device.id]">
                  ...
                  <h5>
                    <DeviceState :value="getObsData(getObservations['1'+device.id.toString().padStart(5,0)+'00'])" :sub="getObservations['1'+device.id.toString().padStart(5,0)+'04'] ?
                getObservations['1'+device.id.toString().padStart(5,0)+'04'].nvalue : 0" text="초 남음" />
                  ...
                  <div style="text-align:center" class="mb-3">
                    <el-popover trigger="hover" placement="top-start" v-if="device.spec.Type.split('/')[1] === 'level2'">
                      <template>
                        ...
                        <div v-if="controlData[device.id]">
                          <el-input-number v-model="controlData[device.id].level2.opentime" :min="10"></el-input-number>
                        </div>
                        <el-divider content-position="left">닫기 시간</el-divider>
                        <div>
                          <el-input-number v-model="controlData[device.id].level2.closetime" :min="10"></el-input-number>
                        </div>
                        <div class="mt-3">
                          <el-button style="width:40%;float: right;" @click="controlClick(device,'retractable',cmdCode.retractable.config,'level2')">설정</el-button>
                        </div>
                      </template>
                  ...
                  <div style="text-align:center">
                    <el-button style="width:42%" type="info" slot="reference" plain  @click="controlClick(device,'retractable',cmdCode.retractable.stop)">중지</el-button>
                    <el-button class="ml-4" style="width:22%" type="primary" disabled plain  @click="controlClick(device,'retractable',cmdCode.retractable.open)">열기</el-button>
                    <el-button style="width:22%" type="danger" disabled plain  @click="controlClick(device,'retractable',cmdCode.retractable.close)">닫기</el-button>
                  </div>
                  <div class="mt-3" style="text-align:center">
                    <template  v-if="device.spec.Type.split('/')[1] === 'level1'">
                      <el-divider content-position="left">시간 지정</el-divider>
                      <b-row>
                        <b-col md="6">
                          <el-input-number style="width:100%" :min="10" v-model="controlData[device.id].level1.opentime"></el-input-number>
                          <el-button class="mt-2" style="width:100%" type="primary" plain @click="controlClick(device,'retractable',cmdCode.retractable.timedopen,'level1')">열기</el-button>
                        </b-col>
                        <b-col md="6">
                          <el-input-number style="width:100%" :min="10" v-model="controlData[device.id].level1.closetime"></el-input-number>
                          <el-button class="mt-2" style="width:100%" type="danger" plain @click="controlClick(device,'retractable',cmdCode.retractable.timedclose,'level1')">닫기</el-button>
                        </b-col>
                      </b-row>
                    </template>
                    ...
                    <template v-if="device.spec.Type.split('/')[1] === 'level2'">
                      ...
                      <div class="ml-4 mr-1">
                        <el-slider :min="0" :max="100" v-model="controlData[device.id].level2.position"></el-slider>
                        <el-button style="width:48%;float: right;" type="warning" plain @click="controlClick(device,'retractable',cmdCode.retractable.position,'level2')">시작</el-button>
                      </div>
                    </template>
                  </div>
                </div>
              ...
              <b-col md="8">
                <div class="mb-2">
                  ...
                  <el-switch style="float:right" v-model="device.roll" active-text="사용" inactive-text="롤링 중지"></el-switch>
                </div>
                <TimeLine ref="timeline" type="switch" :roll="device.roll" :history="history[device.id]" :currentState="getObservations[device.id * 100 + 10000000]" />
              </b-col>
            </b-row>
          </b-card>
        </b-col>
            ...
            <b-row>
              <b-col md="4">
                ...
                getObservations['1'+device.id.toString().padStart(5,0)+'04'].nvalue : 0" text="초 남음" />
                  ...
                  <el-divider content-position="left">기본 제어</el-divider>
                  <div style="text-align:center">
                    <el-button style="width:45%;" type="primary" plain @click="controlClick(device,'switch',cmdCode.switch.on)">시작</el-button>
                    <el-button slot="reference" style="width:45%;float:right;" type="info" plain @click="controlClick(device,'switch',cmdCode.switch.off)">중지</el-button>
                  </div>
                  <div class="mt-3" style="text-align:center">
                    <template v-if="device.spec.Type.split('/')[1]!=='level0'">
                      <el-divider content-position="left">시간 지정</el-divider>
                      <div>
                        <el-input-number :min="10" style="width:45%;" v-model="controlData[device.id].level1.holdtime"></el-input-number>
                        <el-button class="ml-2" style="width:45%;float:right;" type="primary" plain @click="controlClick(device,'switch',cmdCode.switch.timedon,'level1')">시작</el-button>
                      </div>
                    </template>
                    <template v-if="device.spec.Type.split('/')[1]==='level2'">
                      <el-divider content-position="left">비율 시간 지정</el-divider>
                      <div class="ml-4 mr-1">
                        <el-slider :min="0" :max="100" v-model="controlData[device.id].level2.ratio"></el-slider>
                      </div>
                      <div>
                        <el-input-number style="width:45%" v-model="controlData[device.id].level1.holdtime"></el-input-number>
                        <el-button style="width:45%;float:right;" type="primary" plain @click="controlClick(device,'switch',cmdCode.switch.directionalon,'level2')">시작</el-button>
                      </div>
                    </template>
                  </div>
                </div>
            ...
            <b-row>
              <b-col md="4">
                <div v-if="controlData[device.id]">
                  ...
                  <div class="mb-2">제어권 : {{getObservations['1'+device.id.toString().padStart(5,0)+'06'].nvalue === 1 ? '로컬' : '플랫폼'}}</div>
                  <div>진행 구역</div>
                  <div class="mt-1 mb-3">
                    <el-tag :key="place" v-for="(place,index) in device.place" size="small" class="mr-1" :type="getObservations['1'+device.id.toString().padStart(5,0)+'07'].nvalue === index+1 ? 'primary' : 'info'"
                      :effect="getObservations['1'+device.id.toString().padStart(5,0)+'07'].nvalue === index+1 ? 'dark' : 'plain'">{{getFieldName(index,place)}}</el-tag>
                  </div>
                  <h5>
                    <DeviceState :value="getObsData(getObservations['1'+device.id.toString().padStart(5,0)+'00'])" />
                  </h5>
                  <el-divider content-position="left">기본 제어</el-divider>
                  ...
                  <div class="mt-3" style="text-align:center">
                    <el-button style="width:48%;" type="primary" plain @click="controlClick(device,'nutrient-supply',cmdCode.nutrient.on)">시작</el-button>
                    <el-button slot="reference" style="width:48%" type="info" plain @click="controlClick(device,'nutrient-supply',cmdCode.nutrient.off)">중지</el-button>
                    ...
                    <template v-if="device.spec.Type.split('/')[1] !== 'level0'">
                      ...
                      <div class="mt-3">
                        ...
                        <b-row class="mt-1">
                          <b-col md="6" class="p-0">
                            <el-select placeholder="시작구역" style="width:100%" v-model="controlData[device.id].level1.startarea">
                              <template v-for="(place,placeIndex) in device.place">
                                <template v-for="field in getFields">
                                  <el-option v-if="place === field.id" :key="place+field" :label="(placeIndex+1) +' - ' +field.name" :value="field.id"></el-option>
                                </template>
                              </template>
                            </el-select>
                          </b-col>
                          <b-col md="6" class="p-0">
                            <el-select placeholder="종료구역" :style="{width:'97%' ,float:'right'}" v-model="controlData[device.id].level1.stoparea">
                              <template v-for="(place,placeIndex) in device.place">
                                <template v-for="field in getFields">
                                  <el-option v-if="place === field.id" :key="place+field" :label="(placeIndex+1) +' - ' +field.name" :value="field.id"></el-option>
                                </template>
                      ...
                      <div class="mt-3">
                        ...
                        <el-input style="width:48%;" class="ml-2" v-model="controlData[device.id].level1.ph">
                          <template slot="prepend">pH&nbsp;</template>
                        </el-input>
                      </div>
                      <div class="mt-2">
                        <el-input :style="{width:'50%' ,float:'left'}" v-model="controlData[device.id].level1.sec">
                          <template slot="prepend" style="width:40px;">시간</template>
                        </el-input>
                        <el-button :style="{width:'48%' ,float:'right'}" type="primary" plain @click="controlClick(device,'nutrient-supply',cmdCode.nutrient.param,'level1')">시작</el-button>
                      </div>
                    </template>
                  </div>
                </div>
              ...
              <b-col md="8">
                ...
                <TimeLine ref="timeline" type="switch" :history="history[device.id]" :currentState="getObservations[device.id * 100 + 10000000]" />
              </b-col>
            </b-row>
          </b-card>
        </b-col>
...
export default {
  ...
  methods: {
    ...
    async getDeviceHistory (deivceId, date) {
      const {
        data
      } = await this.axios.get(`device/history/${deivceId}`, {
        params: {
